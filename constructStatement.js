...
export default function constructStatement(
  ...
  const getResource = (resources, region, module, type, name) => {
    // console.log(`region:${region}, module:${module}, type:${type}, name:${name}`)
    if (module) {
  ...
  if (!resource) {
    notFounds.push(address)
    return [notFounds, commands]
  }

  const instances = resource.instances
  const modulePrefix = resource.module && resource.module.split('.').length > 2
  ...
  _.forEach(instances, (x) => {
    switch (type) {
      ...
      case 'aws_cloudwatch_dashboard':
        commands.push(`${importCmd} '${type}.${name}' "${x.attributes.id}"`)
        break

        case 'aws_sns_topic_subscription':
          commands.push(`${importCmd} '${resourceAddr}${getKey(x.index_key)}' "${x.attributes.arn}"`)
          break

      case 'pagerduty_service_integration':
        commands.push(`${importCmd} '${resourceAddr}${getKey(x.index_key)}' "${x.attributes.service}.${x.attributes.id}"`)
        break
